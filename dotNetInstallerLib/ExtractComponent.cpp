...
{
	std::wstring resname = GetResName();
...
bool ExtractComponent::HasResource() const
{
	std::wstring resname = GetResName();
	HRSRC l_res = FindResource(m_h, resname.c_str(), TEXT("RES_CAB"));
	return (l_res == NULL) ? false : true;
}
...
std::wstring ExtractComponent::GetResName() const
{
	std::wstring resname = TEXT("SETUP");
