...
namespace DVLib
{
	...
	inline bool FileExistsCustom(LPCTSTR filename)
	{
		...
		/* This methods seems to not work with directory
		HANDLE l_hFile = CreateFile(filename,
									0,  //If this parameter is zero, the application can query file and device attributes without accessing the device. It can also be used to test for the file's or directory's existence without opening it for read or write access.
	...
	// Path Separators: \ or /
	inline CString GetFileNameFromFullFilePath(LPCTSTR p_FileFullPath)
	{
	...
	// Path Separators: \ or /
	inline CString GetPathFromFullFilePath(LPCTSTR p_FileFullPath)
	{
		CString l_FullPath = p_FileFullPath;
	...
	inline CString PathCombineCustom(LPCTSTR p_Path1, LPCTSTR p_Path2, char p_separator = '\\')
	{
