///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
// Origional version from http://www.schneier.com/code/bfsh-unk.zip
// Origional author: Unknown
//
// Modified by Elmü (http://netcult.ch/elmue)
// Date: 20-06-2008
//
//
// Filename: Blowfish.hpp
//
// Classes:
// - CBlowfish
//
// Purpose: Blowfish encryption and decryption
// Blowfish is a very fast symmetrical algorithm.
// Blowfish is unpatented and license-free.
// This class is optimized for maximum speed.
// This class is optimized for Windows. (support for big-endian processors removed)
// Blowfish encrypts/decrypts independent blocks of 8 Bytes which allows random access to the data.
// This class can also encrypt a continous stream.
//
...
public:
	...
	// Passing u32_KeyLen = 0 turns off encryption/decryption (all functions will return FALSE)
	// ATTENTION:
	...
	BOOL CBlowfish::SetPassword(void* p_Key, DWORD u32_KeyLen)
	{
		...
		int i,j = 0;
		for (i=0; i<P_ARRAYS; i++)
		{
			Data32 Pwd;
			Pwd.Byte[3] = ((BYTE*)p_Key)[(j++) % u32_KeyLen];
			Pwd.Byte[2] = ((BYTE*)p_Key)[(j++) % u32_KeyLen];
			Pwd.Byte[1] = ((BYTE*)p_Key)[(j++) % u32_KeyLen];
			Pwd.Byte[0] = ((BYTE*)p_Key)[(j++) % u32_KeyLen];
			PArr[i] ^= Pwd.Val;
		}
		...
		for (i=0; i<P_ARRAYS; i+=2)
		{
			EncryptBlock(&Init);
			PArr[i]   = Init.Uint[0].Val;
			PArr[i+1] = Init.Uint[1].Val;
		}
	
		for (j=0; j<S_BOXES; j++)
		{
			for (i=0; i<BOX_ENTRIES; i+=2)
			{
				EncryptBlock(&Init);
				SBox[j][i]   = Init.Uint[0].Val;
				SBox[j][i+1] = Init.Uint[1].Val;
			}
		}
	...
	// Encryption / Decryption function which allows randam access in blocks of 8 Bytes.
	...
	BOOL CBlowfish::EncryptStreamOpen()
	{
		...
		// The byte pointer must be initialized for every stream anew!
		ms32_BlockPos = 0;
		return TRUE;
	}
	
	// Encrypts the sequential stream data in buffer u8_Data and writes it to buffer u8_Crypt.
	// The encrypted data may be up to 7 Bytes longer than the input data because encryption
	// always uses a fixed block length of 8 Bytes.
	// If for example 6 Bytes of data are passed, nothing will be returned until more data arrives.
	// The residuum will be stored in mu64_Block until the block is full and flushed to the output buffer.
	...
	{
		// StreamOpen() / SetPassword() have not been called
		if (ms32_BlockPos < 0)
			return FALSE;
	
		// The output buffer must be big enough to hold an additional block
		if (u32_CryptLen < u32_DataLen + 8)
			return FALSE;
	
		for (DWORD B=0; B<u32_DataLen; B++)
		{
			mu64_Block.Byte[ms32_BlockPos++] = u8_Data[B];
	...
	// This flushes the remaining encryption bytes which are stored in mu64_Block.
	// The bytes which are unused in the last block are zeroed.
							           ...
							           DWORD* pu32_Written)  // the number of bytes written to output buffer are added up here
	{
		...
		for (int B=0; B<ms32_BlockPos; B++)
		{
			pu64_LastBlock->Byte[B] = mu64_Block.Byte[B];
		}
	
		EncryptBlock(pu64_LastBlock);
...
private:
	...
	Data64 mu64_Block;    // Residuum from the last encryption
	int    ms32_BlockPos; // Current write position in mu64_Block
	...
	void CBlowfish::Initialize()
	{
		...
		const DWORD SBox_Init[S_BOXES][BOX_ENTRIES] = // 4 * 256-entry S-boxes
		{
	...
	#define F(Dat32)(((SBox[0][Dat32.Byte[3]] + SBox[1][Dat32.Byte[2]]) ^ SBox[2][Dat32.Byte[1]]) + SBox[3][Dat32.Byte[0]])
	...
	inline void CBlowfish::EncryptBlock(Data64* pu64_Block)
	{
		...
		pu64_Block->Uint[0] = Hi;
		pu64_Block->Uint[1] = Lo;
	}
