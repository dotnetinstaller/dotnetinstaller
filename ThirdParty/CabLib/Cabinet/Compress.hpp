...
public:
	...
	BOOL AddFolderW(/*NO const*/ CStrW sw_Path, const CStrW& sw_Filter=L"*.*", eCompress e_Compress=E_ComprLZX, int s32_BaseLen=-1)
	{
		...
		do
		{
			...
			{
				if (!AddFolderW(sw_File, sw_Filter, e_Compress, s32_BaseLen))
				{
					FindClose(h_File);
					return FALSE;
				}
			...
			{
				if (!AddFileW(sw_File, (WCHAR*)sw_File + s32_BaseLen, e_Compress))
				{
					FindClose(h_File);
					return FALSE;
				}
