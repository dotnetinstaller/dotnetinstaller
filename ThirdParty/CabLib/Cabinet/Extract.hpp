...
public:
    ...
    struct kCallbacks
    {
        ...
        t_BeforeCopyFile f_OnBeforeCopyFile; // This callback may modify pk_Info->u16_FullPath -> extract into the returned full path
    ...
    {
        #if _TraceExtract
            CTrace::TraceW(L"*** CleanUp() ***");
        #endif

        BOOL b_Ret = DestroyFDIContext();

        // Always remove "this" pointer from TLS memory (even in case of an error in DestroyFDIContext)
        This(TRUE, NULL);
    ...
    // You may change the default "CRYP" to other characters here but don't forget to do the same in CCompress, too!
    ...
    BOOL DestroyFDIContext()
    {
        ...
        if (mh_FDIContext)
        {
            b_Ret = mf_FdiDestroy(mh_FDIContext);
            mh_FDIContext = 0;
        }
        return b_Ret;
    }
    ...
    BOOL IsCabinetW(const CStrW& sw_CabPath, PFDICABINETINFO pfdici = NULL)
    {
        ...
        // bRet == TRUE -> CAB file is OK
        BOOL bRet = mf_FdiIsCabinet(mh_FDIContext, fd, pfdici);

        // Some additional checks may be usefull, because FdiIsCabinet() 
        // sometimes does not detect encrypted archives with the wrong password.
        // To make the detection of a wrong decryption password bullet-proof you must set ENCRYPTION_START = 8
        if (bRet && ENCRYPTION_START == 8)
        {
            // Read the CAB file header
            FdiSeek(fd, 0, SEEK_SET);

            BYTE u8_Header[0x38] = {0};

            // Read the decrypted CAB file header
            int s32_Read = FdiRead(fd, u8_Header, sizeof(u8_Header));
            if (s32_Read != sizeof(u8_Header))
                bRet = FALSE;
            
            // Get the length of the file stored in the CAB header
            DWORD u32_FileLen = *((DWORD*)(u8_Header + 8));

            // If the CAB file is encrypted, the file length is padded to an 8 byte boundary.
            u32_FileLen = (u32_FileLen + 7) & (~7); // Kill the last 3 bits

            // Get the length of the file on disk
            DWORD u32_RealLen = FdiSeek(fd, 0, SEEK_END);

            // If the file length does not match, the file is not a CAB file or corrupt or encrypted with another password
            if (u32_FileLen != u32_RealLen)
                bRet = FALSE;
        }
...
private:
    ...
    { 
        ...
        if (s32_CabPtr == 0 && s32_Read >= 4) // memory contains the first bytes of a CAB file
        {
            ...
            else if (strncmp((char*)memory, ms8_CryptBOM, 4) == 0)
            {
                if (!mi_Blowfish.IsPasswordSet())
                {
    ...
    INT_PTR FdiCallback(FDINOTIFICATIONTYPE fdint, PFDINOTIFICATION pfdin)
    {
        ...
        switch (fdint)
        {
            ...
            case fdintCOPY_FILE:
            {
                ...
                {
                    // The callback is allowed to modify k_FI.u16_FullPath which will extract into the modified path.
                    // Reserve enough space here!
                    sw_FullPath.Allocate(MAX_PATH);
                ...
                {
                    CFile::SplitPathW(sw_FullPath, &sw_Path, NULL);
            ...
            case fdintCLOSE_FILE_INFO:
            {
                ...
                // sw_FullPath may have been manipulated by the OnBeforeCopyFile callback
                if (pfdin->hf == mk_CurrentFile.h_File)
                {
                    sw_Path = mk_CurrentFile.sw_FullPath;
                }
                else
                {
                    CStrW sw_RelPath;
                ...
                #if _TraceExtract
                    CStrW sw_FileName;
                    CFile::SplitPathW(sw_Path, 0, &sw_FileName);
