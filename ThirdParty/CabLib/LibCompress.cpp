...
namespace CabLib
{
    ...
    void CabLib::Compress::SetEncryptionKey(System::Byte u8_Key[])
    { 
        mu8_Key = u8_Key;
    }
        ...
        Int32 s32_SplitSize) // may be 0
    {
        ...
        for (int i=0; i<i_FileList->Count; i++)
        {
            ...
            String* s_Items[] = static_cast<String*[]>(Array::CreateInstance(__typeof(String), 2));

            s_Items[0] = static_cast<String*>(i_FileList->Item[i]); // File name
            s_Items[1] = s_Items[0]->Substring(s32_BaseFolderLen);  // Cut base path -> rel path in CAB

            i_FileList->Item[i] = s_Items;
        }
    ...
    void CabLib::Compress::CompressFileList(ArrayList* i_FileList, 
        ...
        Int32    s32_SplitSize) // may be 0
    {
        if (mi_Compress) throw new Exception(new String("You cannot call CompressFileList() from two different threads. Please create a new instance for each thread!"));
        ...
        cCallbackBridge i_Bridge(evUpdateStatus, evFilePlaced);
        ...
        {
            i_Ex = new Exception(String::Format("CabLib Compress ERROR: Error setting temp directory: {0}",
                new String(i_Compress.LastErrorW())));
            goto _Exit;
        }
        ...
        {
            i_Ex = new Exception(String::Format("CabLib Compress ERROR: Could not create FCI context: {0}", 
                new String(i_Compress.LastErrorW())));
            goto _Exit;
        }
        ...
        for (int i=0; i<i_FileList->Count; i++)
        {
            String* s_Item[] = static_cast<String*[]>(i_FileList->Item[i]);
            if (s_Item->Length != 2)
            {
                i_Ex = new Exception("Invalid Parameter: CompressFileList() requires an ArrayList with string pairs. Read the documentation!");
                goto _Exit;
            }
            ...
            Marshal::FreeHGlobal(u16_PathInCab);

            if (!b_OK)
            {
                i_Ex = new Exception(String::Format("CabLib Compress ERROR: Could not add file \"{0}\" to cabinet: {1}", 
                    s_Item[0], new String(i_Compress.LastErrorW())));
                goto _Exit;
            }
        }
        ...
        {
            i_Ex = new Exception(String::Format("CabLib Compress ERROR: Could not flush cabinet: {0}", 
                new String(i_Compress.LastErrorW())));
            goto _Exit;
        }
...
_Exit:
        ...
        Marshal::FreeHGlobal(u16_TempDir);
        ...
        bool       b_Subfolders)
    {
        ...
        System::Char s8_Pipe[] = { '|' };
        String* s_ExtList[] = s_Filter->Split(s8_Pipe);
        for (int i=0; i<s_ExtList->Length; i++)
        {
            String *s_Ext = static_cast<String*>(s_ExtList->Item[i]);
    ...
    String* CabLib::Compress::Sprintf(String* s_Format, Object* o_Values[])
    {
        ...
        {
            Marshal::GetNativeVariantForObject(o_Values[i], pk_Args + i);
        }
    ...
    BOOL CabLib::Compress::cCallbackBridge::OnGetNextCabinet(PCCAB pccab, ULONG cbPrevCab, char* s8_CabNameFormatter, void* p_Param)
    { 
        // Modify if required:
        // pccab->iCab = xyz
        // pccab->iDisk = xyz
    ...
    int CabLib::Compress::cCallbackBridge::OnUpdateStatus(UINT typeStatus, Cabinet::CCompress::kCurStatus *pk_CurStatus, void* p_Param)
    {
        ...
        kCurStatus* k_Status = new kCurStatus();
        ...
        return p_Bridge->mev_UpdateStatus->Invoke(k_Status);
    }
    ...
    int CabLib::Compress::cCallbackBridge::OnFilePlaced(CCAB *pccab, WCHAR* u16_File, int s32_FileSize, BOOL fContinuation, void* p_Param)
    {
        if (!p_Param) // May happen if destructor of CCompress calls DestroyFciContext()
            return 0; 
        ...
        return p_Bridge->mev_FilePlaced->Invoke(new String(u16_File), s32_FileSize, (fContinuation != FALSE)); // BOOL --> bool
    }
