...
union ClassesAllowedInStream 
{
	...
	HRESULT (*pfnClsidAllowed)(__in const CLSID& clsid, __in REFIID iidInterface, __deref_inout_opt void** ppvObj);
};

#if !defined(_ATL_DLL_IMPL)
inline HRESULT AtlInternalOleLoadFromStream(
	__inout IStream* pStm, 
	__in REFIID iidInterface, 
	__deref_out_opt void** ppvObj, 
	__in ClassesAllowedInStream rgclsidAllowed, 
	__in DWORD cclsidAllowed);
#endif
...
public:
	...
	HRESULT ReadFromStream(__inout IStream* pStream, __in VARTYPE vtExpected,
		__in ClassesAllowedInStream rgclsidAllowed, __in DWORD cclsidAllowed);
...
inline HRESULT CComVariant::ReadFromStream(__inout IStream* pStream, __in VARTYPE vtExpected /* = VT_EMPTY */ )
{
...
inline HRESULT CComVariant::ReadFromStream(__inout IStream* pStream, __in VARTYPE vtExpected, 
				__in ClassesAllowedInStream rgclsidAllowed, __in DWORD cclsidAllowed)
{
	ATLASSERT(pStream != NULL);
	if(pStream == NULL)
