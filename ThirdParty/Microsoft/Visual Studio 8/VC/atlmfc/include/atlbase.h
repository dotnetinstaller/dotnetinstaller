...
#include <atlassem.h>
	...
	#pragma comment(lib, "atlmincrt.lib")
		...
		#if defined(_M_IX86)
			#pragma comment(linker, "/include:__forceAtlDllManifestRTM")
		#else
			#pragma comment(linker, "/include:_forceAtlDllManifestRTM")
		#endif
	#else
		#if defined(_M_IX86)
...
ATLINLINE ATLAPI AtlRegisterTypeLib(HINSTANCE hInstTypeLib, LPCOLESTR lpszIndex)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			Checked::wcsncpy_s(szDir, MAX_PATH, bstrHelpFile.m_str, bstrHelpFile.Length());
			szDir[MAX_PATH - 1] = 0;
