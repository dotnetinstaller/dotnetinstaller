...
public:
	...
	static UINT CreateFolderTreeA(char* s8_DirTree)
	{
		...
		char   s8_Folder[1000];
		char   s8_Parent[1000];
		strcpy_s(s8_Folder, sizeof(s8_Folder), s8_DirTree);
		strcpy_s(s8_Parent, sizeof(s8_Parent), s8_DirTree);
	...
	{
		size_t s32_Len = wcslen(u16_Path);
