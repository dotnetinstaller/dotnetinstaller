...
namespace Cabinet
{
    ...
    public:
        ...
        void Replace(WCHAR u16_Old, WCHAR u16_New)
        {
            UINT u32_Len = Len();
            for (UINT i=0; i<u32_Len; i++)
            {
                if (mu16_Buf[i] == u16_Old) 
                    mu16_Buf[i] =  u16_New;
            }
        }
        ...
        // Convert an ANSI/UTF8/Codepage string into an Unicode string and stores it in "this"
        WCHAR* ToUnicode(DWORD u32_Codepage, const char* s8_String)
        {
            // UTF8: One Unicode character is converted into 1,2,3 or 4 ASCII characters
            mu16_Buf[0] = 0;		
            Allocate((UINT)strlen(s8_String));

            int s32_Len = MultiByteToWideChar(u32_Codepage, 0, s8_String, -1, mu16_Buf, mu32_Size);
